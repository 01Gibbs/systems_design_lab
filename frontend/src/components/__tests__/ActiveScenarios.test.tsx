...
describe('ActiveScenarios', () => {
  ...
  it('properly renders JSX fragment for expires information', () => {
    ...
    expect(expiresText?.textContent).toMatch(
      /\d{1,2}\/\d{1,2}\/\d{4}.*Expires:.*\d{1,2}\/\d{1,2}\/\d{4}/
    );
  });
  ...
  it('correctly handles complex parameter objects', () => {
    ...
    const preElement = screen.getByText(
      (text) =>
        text.includes('"nested"') && text.includes('"array"') && text.includes('"boolean": true')
    );
